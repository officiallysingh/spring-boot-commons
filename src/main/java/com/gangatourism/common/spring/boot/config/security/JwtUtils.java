...
class JwtUtils {
  ...
  private static Converter<Map<String, Object>, Map<String, Object>> claimSetConverter =
  ...
  private static Jwt createJwt(String token, JWT parsedJwt) {
    try {
      ...
      // @formatter:off
      Builder jwtBuilder =
          Jwt.withTokenValue(token).headers(h -> h.putAll(headers)).claims(c -> c.putAll(claims));
